...
(def axis {"x-axis" {"label" {"alignment" (fn [chart chart-map]
                                            (-> (.getStyler chart)
                                                (.setXAxisLabelAlignment (get axis-alignment (get-in chart-map ["alignment"]))))
                                            chart)
                              "rotation"  (fn [chart chart-map]
                                            (-> (.getStyler chart)
                                                (.setXAxisLabelRotation (int (get-in chart-map ["rotation"]))))
                                            chart)}
           ...
           "y-axis" {"decimal-pattern" (fn [chart chart-map]
                     ...
                     "label"           {"alignment" (fn [chart chart-map]
                                                      (-> (.getStyler chart)
                                                          (.setYAxisLabelAlignment (get axis-alignment (get-in chart-map ["alignment"]))))
                                                      chart)
                                        "rotation"  (fn [chart chart-map]
                                                      (-> (.getStyler chart)
                                                          (.setYAxisLabelRotation (int (get-in chart-map ["rotation"]))))
                                                      chart)}
