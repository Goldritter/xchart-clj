...
(def chart-functions {:title                            {:visible? (fn [chart chart-map]
                      ...
                      :plot-background-color            (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setPlotBackgroundColor (get-color (:plot-background-color series-map))))
                                                          chart)

                      :plot-gridlines-color             (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setPlotGridLinesColor (get-color (:plot-gridlines-color series-map))))
                                                          chart)

                      :chart-background-color           (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setChartBackgroundColor (get-color (:chart-background-color series-map))))
                                                          chart)
                      :legend-background-color          (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setLegendBackgroundColor (get-color (:legend-background-color series-map))))
                                                          chart)
                      :chart-font-color                 (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setChartFontColor (get-color (:chart-font-color series-map))))
                                                          chart)
                      :chart-title-box-background-color (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setChartTitleBoxBackgroundColor (get-color (:chart-title-box-background-color series-map))))
                                                          chart)

                      :chart-title-box-border-color     (fn [chart series-map]
                                                          (-> (.getStyler chart)
                                                              (.setChartTitleBoxBorderColor (get-color (:chart-title-box-border-color series-map))))
                                                          chart)
...
(defn adjust-axis
  ...
  (reduce #(if (map? (val %2))
             (adjust-chart %1 ((key %2) axis-map) (val %2))
             ((val %2) %1 axis-map))
          chart
          (select-keys (used-axis axis) (keys axis-map))))
...
(defn generate-chart [chart-map]
  (let [used-chart-map (transient chart-map)
    ...
    (adjust-axis chart (:x-axis used-chart-map) :x-axis)
    (adjust-axis chart (:y-axis used-chart-map) :y-axis)
